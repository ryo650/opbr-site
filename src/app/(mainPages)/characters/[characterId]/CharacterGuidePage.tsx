...
function PointGrid({ points, collapseDescriptions = false }: { points: GuidePoint[]; collapseDescriptions?: boolean }) {
  return <div className={styles.pointGrid}>{points.map((point) => <article className={styles.card} key={point.title}><h3>{point.title}</h3>{collapseDescriptions ? <CollapsibleText>{point.description}</CollapsibleText> : <p>{point.description}</p>}</article>)}</div>;
}
...
function MatchupCard({ matchup, character, kind }: MatchupProps) {
  ...
  return <article className={styles.matchupCard}>
    ...
    <CollapsibleText>{matchup.reason}</CollapsibleText>
  </article>;
}
...
export default function CharacterGuidePage({ character, guide, matchupCharacters }: { character: Character; guide: CharacterGuide; matchupCharacters: Record<string, Character> }) {
  ...
  return <main className={`${styles.page} upper-page-background`}>
    <article className={styles.content}>
      ...
      {!!guide.strengths?.length && <Section title="Strengths"><PointGrid points={guide.strengths} collapseDescriptions /></Section>}
      {!!guide.weaknesses?.length && <Section title="Weaknesses"><PointGrid points={guide.weaknesses} collapseDescriptions /></Section>}
      {!!guide.normalAttacks?.length && <Section title="Normal Attacks"><div className={styles.mediaGrid}>{guide.normalAttacks.map((attack, index) => attack.video && <ViewportVideo key={attack.video} src={attack.video} label={`${character.name} normal attacks${index ? ` ${index + 1}` : ""}`} />)}</div></Section>}
      {!!guide.skills?.length && <Section title="Skills"><div className={styles.skillGrid}>{guide.skills.map((skill) => <article className={`${styles.card} ${styles.skillCard}`} key={skill.name}><div className={styles.skillTitle}><h3>{skill.name}</h3>{skill.cooldown !== undefined && <span>{skill.cooldown}s cooldown</span>}</div>{skill.video && <ViewportVideo src={skill.video} label={`${skill.name} demonstration`} />}<CollapsibleText>{skill.description}</CollapsibleText>{!!skill.quickTips?.length && <div className={styles.tips}><h4>Quick Tips</h4><ul>{skill.quickTips.map((tip) => <li key={tip}>{tip}</li>)}</ul></div>}</article>)}</div></Section>}
