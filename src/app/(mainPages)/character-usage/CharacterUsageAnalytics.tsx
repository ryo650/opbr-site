...
export default function CharacterUsageAnalytics({ snapshots, availableCharacters, guideCharacterIds }: Props) {
  ...
  const rows = useMemo(() => buildChartData(filteredSnapshots, availableCharacters.map((character) => character.id)), [availableCharacters, filteredSnapshots]);
  ...
  return <>
    <section className={styles.analytics} aria-labelledby="trends-heading">
      <div className={styles.sectionHeading}><div><p className={styles.eyebrow}>Usage over time</p><h2 id="trends-heading">Estimated player usage</h2></div><div className={styles.chartActions}>{focusedId && <button className={styles.clearFocus} type="button" onClick={() => setFocusedId(null)}>Clear focus</button>}<div className={styles.ranges} aria-label="Chart time range">{ranges.map((item) => <button type="button" key={item} aria-pressed={range === item} disabled={!availableRanges.get(item)} onClick={() => { setRange(item); setTooltipIndex(null); }}>{item}</button>)}</div></div></div>
      <div className={styles.selector}>
        ...
        <div className={styles.selectorList} aria-label="Tracked character results">
          {filteredCharacters.map((character) => {
            ...
            return <button key={character.id} type="button" className={isFocused ? styles.selectedCharacter : ""} aria-pressed={isFocused} onClick={() => setFocusedId(isFocused ? null : character.id)}><Image src={character.image} alt="" width={42} height={42} /><span><strong>{character.name}</strong><small>{character.element} · {character.role}</small></span><span className={styles.selectorStatus}>{isFocused ? "Focused" : isBaseline ? "Top 10" : ""}</span></button>;
          })}
...
function UsageChart({ rows, characters, visibleIds, focusedId, tooltipIndex, setTooltipIndex }: { rows: ReturnType<typeof buildChartData>; characters: Character[]; visibleIds: string[]; focusedId: string | null; tooltipIndex: number | null; setTooltipIndex: (index: number | null) => void }) {
  const chartShellRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    const closeOnEscape = (event: KeyboardEvent) => { if (event.key === "Escape") setTooltipIndex(null); };
    const closeOutsideChart = (event: PointerEvent) => {
      if (chartShellRef.current && !chartShellRef.current.contains(event.target as Node)) setTooltipIndex(null);
    };
    document.addEventListener("keydown", closeOnEscape);
    document.addEventListener("pointerdown", closeOutsideChart);
    return () => {
      document.removeEventListener("keydown", closeOnEscape);
      document.removeEventListener("pointerdown", closeOutsideChart);
    };
  }, [setTooltipIndex]);
  ...
  return <div ref={chartShellRef} className={styles.chartShell} onPointerDown={(event) => { const target = event.target as Element; if (!target.closest("[data-chart-point]") && !target.closest("[data-chart-details]")) setTooltipIndex(null); }}>
    <div className={styles.chartScroll}><svg className={styles.chart} viewBox={`0 0 ${width} ${height}`} role="group" aria-label="Estimated player usage by snapshot date">
      ...
      {rows.map((row, index) => <g key={row.date} data-chart-point onPointerDown={(event) => { event.stopPropagation(); setTooltipIndex(index); }} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setTooltipIndex(index); } }} role="button" aria-pressed={tooltipIndex === index} tabIndex={0} aria-label={`Show ${formatDate(row.date)} chart details`}><line x1={x(index)} x2={x(index)} y1={top} y2={height-bottom} className={styles.hoverColumn} /><text x={x(index)} y={height-17} textAnchor="middle">{formatDate(row.date, true)}</text></g>)}
      {visibleIds.map((id, colorIndex) => { const points = rows.map((row, index) => `${x(index)},${y(row.values[id]?.usageRate ?? 0)}`).join(" "); const focused = id === focusedId; return <g key={id} pointerEvents="none" opacity={focused ? 1 : focusedId ? .58 : .85}><polyline points={points} fill="none" stroke={colors[colorIndex % colors.length]} strokeWidth={focused ? 4 : 2.25} strokeLinejoin="round" strokeLinecap="round" />{rows.map((row, index) => <circle key={row.date} cx={x(index)} cy={y(row.values[id]?.usageRate ?? 0)} r={focused ? 5.5 : 4} fill="#202124" stroke={colors[colorIndex % colors.length]} strokeWidth={focused ? 3 : 2} />)}</g>; })}
    </svg></div>
    {activeRow && <aside data-chart-details className={styles.tooltip} aria-label={`${formatDate(activeRow.date)} usage details`} onPointerDown={(event) => event.stopPropagation()}><header><strong>{formatDate(activeRow.date)}</strong><button type="button" onClick={() => setTooltipIndex(null)} aria-label="Close chart details">×</button></header><div className={styles.tooltipList} tabIndex={0}>{tooltipItems.map((item) => <div key={item.id} className={item.id === focusedId ? styles.tooltipFocused : ""}><span className={styles.tooltipRank}>{item.rank === null ? "—" : `#${item.rank}`}</span><Image src={item.character.image} alt="" width={34} height={34}/><span>{item.character.name}<small>{item.count} recorded</small></span><b style={{ color: colors[item.colorIndex % colors.length] }}>{formatPercent(item.usageRate)}</b></div>)}</div></aside>}
  </div>;
}
