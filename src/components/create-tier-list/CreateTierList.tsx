...
function CharacterCard({ character, isDragging, isPointerDragging, onDragStart, onDragEnd, onPointerStart, onPointerMove, onPointerEnd, onPointerCancel, onSelect }: {
  character: Character; isDragging: boolean; isPointerDragging: boolean; onDragStart: (event: DragEvent<HTMLDivElement>, id: string) => void; onDragEnd: () => void; onPointerStart: (event: PointerEvent<HTMLDivElement>, id: string) => void;
  ...
  return <div className={`${styles.characterCard} ${isDragging ? styles.draggingSource : ""} ${isPointerDragging ? styles.characterCardDragging : ""}`} draggable role="button" tabIndex={0} onDragStart={(event) => onDragStart(event, character.id)} onDragEnd={onDragEnd}
...
export default function CreateTierList() {
  ...
  const pointerStart = useRef<{ x: number; y: number; currentX: number; currentY: number; id: string; pointerId: number; pointerType: string; target: HTMLDivElement; suppressTap: boolean } | null>(null);
  ...
  const clearDragState = () => { clearLongPressTimer(); document.removeEventListener("touchmove", preventTouchScroll); document.body.classList.remove(styles.pointerDraggingBody); dragPreviewElement.current?.remove(); dragPreviewElement.current = null; activeId.current = null; isLongPressDraggingRef.current = false; setDraggingId(null); setDropIntent(null); setPointerPreview(null); };
  ...
  const dragStartHandler = (event: DragEvent<HTMLDivElement>, id: string) => {
    ...
    activeId.current = id; setDraggingId(id); if (pointerStart.current) pointerStart.current.suppressTap = true;
  ...
  const pointerStartHandler = (event: PointerEvent<HTMLDivElement>, id: string) => {
    clearLongPressTimer();
    const start = { x: event.clientX, y: event.clientY, currentX: event.clientX, currentY: event.clientY, id, pointerId: event.pointerId, pointerType: event.pointerType, target: event.currentTarget, suppressTap: false };
    pointerStart.current = start;
    if (event.pointerType !== "touch" && event.pointerType !== "pen") return;
    longPressTimerRef.current = setTimeout(() => {
      if (pointerStart.current !== start) return;
      longPressTimerRef.current = null; isLongPressDraggingRef.current = true; activeId.current = id;
      start.target.setPointerCapture(start.pointerId); document.body.classList.add(styles.pointerDraggingBody);
      document.addEventListener("touchmove", preventTouchScroll, { passive: false });
      setDraggingId(id); setPointerPreview({ id, x: start.currentX, y: start.currentY });
    }, longPressDelay);
  };
  const pointerMoveHandler = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (!isLongPressDraggingRef.current) {
      if (Math.hypot(event.clientX - start.x, event.clientY - start.y) >= scrollMovementThreshold) { clearLongPressTimer(); pointerStart.current = null; }
      return;
    }
    event.preventDefault(); setPointerPreview({ id: start.id, x: event.clientX, y: event.clientY });
  ...
  const pointerEndHandler = (event: PointerEvent<HTMLDivElement>) => {
    const start = pointerStart.current; pointerStart.current = null; clearLongPressTimer();
    if (!start) { clearDragState(); return; }
    if (!isLongPressDraggingRef.current) { clearDragState(); if (!start.suppressTap) setSelectedCharacterId(start.id); return; }
  ...
  return <div className={styles.container}>
    ...
    <div className={`${styles.pool} ${dropIntent?.destination === "pool" && !dropIntent.targetId ? styles.dropAtEnd : ""}`} data-zone-container="pool" onDragOver={(event) => dragOverHandler(event, "pool")} onDragLeave={dragLeaveHandler} onDrop={(event) => dropFromDataTransfer(event, "pool")}>{filteredPool.map((character) => { const indicator = dropIntent?.destination === "pool" && dropIntent.targetId === character.id ? (dropIntent.placement === "before" ? styles.dropBefore : styles.dropAfter) : ""; return <div className={`${styles.characterSlot} ${indicator}`} key={character.id} data-character-id={character.id}><CharacterCard character={character} isDragging={draggingId === character.id} isPointerDragging={pointerPreview?.id === character.id} {...characterCardProps} /></div>; })}{filteredPool.length === 0 && <p className={styles.empty}>No unranked characters match your search and filters.</p>}</div></section>
