...
function CharacterCard({ character, onDragStart, onTouchStart, onTouchMove, onTouchEnd }: {
  character: Character; onDragStart: (event: DragEvent<HTMLDivElement>, id: string) => void; onTouchStart: (event: PointerEvent<HTMLDivElement>, id: string) => void;
  onTouchMove: (event: PointerEvent<HTMLDivElement>) => void; onTouchEnd: (event: PointerEvent<HTMLDivElement>) => void;
}) {
...
export default function CreateTierList() {
  ...
  const dragStartHandler = (event: DragEvent<HTMLDivElement>, id: string) => { activeId.current = id; event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", id); };
  ...
  const touchStartHandler = (event: PointerEvent<HTMLDivElement>, id: string) => { if (event.pointerType === "mouse") return; activeId.current = id; touchStart.current = { x: event.clientX, y: event.clientY, moved: false }; event.currentTarget.setPointerCapture(event.pointerId); };
  const touchMoveHandler = (event: PointerEvent<HTMLDivElement>) => { const start = touchStart.current; if (start && Math.hypot(event.clientX - start.x, event.clientY - start.y) > 8) start.moved = true; };
  const touchEndHandler = (event: PointerEvent<HTMLDivElement>) => { const start = touchStart.current; touchStart.current = null; if (!start?.moved) { activeId.current = null; return; } const target = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>("[data-drop-zone]"); if (target?.dataset.dropZone) drop(target.dataset.dropZone as DropZone, target.dataset.characterId); else activeId.current = null; };
