...
export function isCompleteFeaturedOnlyFourStarPool({
  ...
  return ratesMatchWithinDisplayPrecision(pickupTotal, totalFourStarRate);
}
...
export function calculateScoutRates({
  ...
  if (
    ...
    !ratesMatchWithinDisplayPrecision(pickupTotal, totalFourStarRate)
  ) {
    throw new Error("A featured-only ★4 pool requires pickup total to match ★4 total within display precision");
  }
...
export function calculateFinalScoutRates({
  ...
  const observedPickupTotal = sumDecimals(pickups.map(({ rate }) => rate));
  // The game truncates repeating individual rates to seven decimals. When the
  // complete ★4 pool is featured, keep those displayed weights on each pickup
  // but use the authoritative ★4 summary as the category rate.
  const pickupTotal = rateCalculation.bfCountSource === "complete featured ★4 pool"
    ? totalFourStarRate
    : observedPickupTotal;
...
function renderScoutModuleSource(draft) {
...
export function renderScoutModule(draft) {
  const source = renderScoutModuleSource(draft);
  if (!draft.featuredOnlyFourStarPool) return source;
  return source
    .replace(
      /const totalPickupRate = pickups\.reduce\([\s\S]*?\);\n\n/,
      "",
    )
    .replace(
      "pickup: totalPickupRate,",
      `pickup: ${decimalToString(draft.finalRates.pickupTotal)},`,
    );
}
